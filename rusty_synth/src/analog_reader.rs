...
pub struct AnalogReaderAdc2<PIN: AdcChannel + AnalogPin, F: Fn(u16) -> f32> {
    adc_pin: AdcPin<PIN, ADC2, AdcCalLine<ADC2>>,
    activation: F,
    accumulator: u16,
}
...
impl<PIN, F> AnalogReaderAdc2<PIN, F>
where
    PIN: AdcChannel + AnalogPin,
    F: Fn(u16) -> f32,
{
    pub fn new(
        pin: PIN,
        activation: F,
        adc_config: &mut AdcConfig<ADC2>,
    ) -> AnalogReaderAdc2<PIN, F> {
        let adc_pin =
            adc_config.enable_pin_with_cal::<PIN, AdcCalLine<_>>(pin, Attenuation::Attenuation11dB);
        AnalogReaderAdc2 {
            adc_pin,
    ...
    pub fn read(&mut self, driver: &mut Adc<ADC2>) -> f32 {
        let value = driver.read_blocking(&mut self.adc_pin);
        //exponential moving average (smoothing / (window_size + 1))
        let alpha = 2.0 / 257.0;
        self.accumulator =
